...
def range_diffplot(plotType):
    ...
    if plotType == "ThresholdMap-0":
        ...
        print 'MapType = {} is not registed. Set default range = [100,-500,500]...... '
...
def check(a,b):
    if a != b:
        ...
        sys.exit()
def diff_pixelconfig(a,b,par1,par2):
    if par1 == par2 :
        h3_0.Fill(a-b)
    elif par1-par2==1 :
        h3_1.Fill(a-b)
...
DAT_file_1='{0}/*{1}_*/*.dat'.format(TMP_DIR,args[1]) #list
DAT_file_2='{0}/*{1}_*/*.dat'.format(TMP_DIR,args[2]) #list
Config_file_1='{0}/*{1}_*/*.before'.format(TMP_DIR,args[1])
Config_file_2='{0}/*{1}_*/*.before'.format(TMP_DIR,args[2])
...
information_num=8
print files_1
datafile=ROOT.TFile('Diff_{0}_{1}.root'.format(args[1],args[2]),"recreate");
#pixel_tree=ROOT.TTree("pixel_tree","")

file_count=0
precanvas=ROOT.TCanvas("precanvas","")
precanvas.Print('{0}_{1}.pdf['.format(run_1,run_2))
...
#                            print parameter2
                            if int(xrange[0]) == 80 and int(xrange2[0]) == 80:
                                diff_pixelconfig(float(parameter),float(parameter2),float(cf_json_1['FE-I4B']['PixelConfig'][row-1][parName][column-1]),float(cf_json_2['FE-I4B']['PixelConfig'][row-1][parName][column-1]))
...
#                h2.Draw()
                ...
                if int(xrange[0]) == int(xrange2[0]):
                    ...
                    h4.Write()
                    legend.Draw()
                    c4.Print('{0}_{1}.pdf'.format(run_1,run_2))
            h.Write()
            h2.Write()
            print count
c4.Print('{0}_{1}.pdf]'.format(run_1,run_2))
